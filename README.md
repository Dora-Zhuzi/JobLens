...
上传 JD 截图 → OCR 成文字 → AI 结构化/拆解 → 勾选自评匹配度 → 多岗位批量统计，辅助你做求职决策。

---

## 截图

<p align="center"><strong>岗位上传与结构化拆解</strong></p>
<p align="center"><img src="screenshots/01-editor.jpg" alt="岗位上传与结构化拆解" width="82%"></p>

<p align="center"><strong>个人匹配度分析</strong></p>
<p align="center"><img src="screenshots/02-match.jpg" alt="个人匹配度分析" width="82%"></p>

<p align="center"><strong>岗位列表与多维度排序</strong></p>
<p align="center"><img src="screenshots/03-list.jpg" alt="岗位列表与多维度排序" width="100%"></p>

<p align="center"><strong>岗位要求统计</strong></p>
<p align="center"><img src="screenshots/04-stats.jpg" alt="岗位要求统计" width="58%"></p>
...
- **个人匹配度分析**：技能 / 业务经验 / 核心能力 / 性格特质 四维（各 25%），勾选项由岗位要求自动派生（要求改动失焦或自动拆解后同步），勾选即实时计算匹配度，独立黄色色块展示总分与分项。
- **面试策略 / 面试准备**：随内容自适应高度的编辑区，沉淀面试打法与待补足项；面试准备时间以「天」计。
...
- 大模型：DeepSeek（岗位详情结构化、岗位要求自动拆解，纯文本任务）
...
- 两类 key 可只配其一：不配百度则 OCR 回退本地 Tesseract；不配 DeepSeek 则结构化/自动拆解按钮置灰（匹配度为本地计算，不受影响）。
...
| `DEEPSEEK_API_KEY` | DeepSeek 凭证；缺省则禁用 AI 结构化/自动拆解 |
...
├─ styles.css           # 全部样式
├─ app.js               # 前端逻辑（编辑、OCR、AI、匹配度、列表、统计）
├─ server.js            # 本地后端：托管页面 + /api/ocr /structure /jobs
